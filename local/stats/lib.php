...
class stats {
    ...
    function set_data() {
        ...
        $recordsUE = $this->get_user_experience_from_courses($USER->id);
        $doughnotchart = $this->create_experience_chart($recordsUE);

        $recordsattendence = $this->get_user_attendance($USER->id, 604800);
        $attendacechart = $this->create_attendance_chart($recordsattendence);


        $data = new \stdClass();
        $this->experiencechart = $OUTPUT->render($doughnotchart);
        $this->attendacechart = $OUTPUT->render($attendacechart);


        $this->qualified_task = $this->get_last_five_grades_qualificated($USER->id);
        $this->mean = $this->get_mean_time_assigns($USER->id);


        $this->qualificated_post = $this->get_qualifications_from_posts($USER->id);
    }
    ...
    function get_data() {
        ...
        $data->chart1 = $this-> experiencechart;
        $data->chart2 = $this-> attendacechart;
        $data->qualified_task = $this-> qualified_task;
        $data->mean = $this-> mean;
        $data->qualificated_post = $this-> qualificated_post;
    ...
    function get_mean_time_assigns($userid){
        ...
        return $this->seconds_to_days($timemean);
